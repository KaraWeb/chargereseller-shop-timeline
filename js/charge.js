jQuery(document).ready(function ($) {
	...
	function checkForm () {
		...
		if (DefaultChargeKind == 'TopUp') {
			...
			if (cellphone.length == 11 && !isNaN(cellphone)) {
				if ($('div.InternetPackage div.info div#operator').attr('class').replace("operator InternetPackage ", "") == 'IN-MTN-TDLTE') {
		...
		if (cellphoneCheck == false) {
			if ($('div.' + DefaultChargeKind + ' div.' + divType + ' div.input.text.account div.message').length <= 0) {
				$('div.' + DefaultChargeKind + ' div.' + divType + ' div.input.text.account').prepend('<div class="message error-message">شماره وارد شده صحیح نمی باشد.</div>');
			}
		} else {
			$('div.' + DefaultChargeKind + ' div.' + divType + ' div.input.text.account div.message').remove();
		}
		
		if (emailCheck == false) {
		
			if ($('div.' + DefaultChargeKind + ' div.' + divType + ' div.input.text.email div.message').length <= 0) {
				$('div.' + DefaultChargeKind + ' div.' + divType + ' div.input.text.email').prepend('<div class="message error-message">این ایمیل صحیح نمی باشد.</div>');
			}
		} else {
			$('div.' + DefaultChargeKind + ' div.' + divType + ' div.input.text.email div.message').remove();
		}
	...
	setInterval((function() {
		...
		if (DefaultChargeKind == 'PIN') {
			$('input#dataCount').val($('.container.PIN div.charge input#count').val());
			$('input#dataCount').val($('.container.PIN div.charge input#count').val());
		}
		setAmount();
		...
		if (DefaultChargeKind == 'TopUp') {
			...
			$('input#dataAmount').val($('input#dataAmountTemp').val());
			$('.container.PIN div.charge .amount-value').text($('input#dataCount').val() * $('input#dataAmount').val());
		} else if (DefaultChargeKind == 'WiMax') {
			$('input#dataAmount').val($('input#dataAmountTopUpMTNTemp').val());
		}
		...
		$('input#dataEmail').val($('div.' + DefaultChargeKind + ' div.' + divType + ' input[type=email]').val());
		$('input#dataCellphone').val($('div.' + DefaultChargeKind + ' div.' + divType + ' input.cellphone').val());
		
		$('.bgs .center i.timeline').height($('.bgs > .container').height() - 250);
	}), 200);
	...
	$('.services-menu .service-container').bind('mouseenter',function(){
		var element = $(this);
		element.find('.service-logo')
			 .stop(true)
			 .animate({
				'width':'118px',
				'height':'85px',
			...
			var subMenu = element.find('.sdt_box');
			if(subMenu.length){
				var left = '170px';
				if(element.parent().children().length == element.index()+1)
					left = '-170px';
		...
		var subMenu = element.find('.sdt_box');
		if (subMenu.length)
			subMenu.hide().css('left','0px');
		
		element.find('.sdt_active')
			 .stop(true)
			 .animate({'height':'0px'},300)
			 .andSelf().find('.service-logo')
			 .stop(true)
			 .animate({
				'width':'0px',
				'height':'0px',
	...
	$('.services-menu .service-container').on('click', function(){
        ...
        if (jQuery.inArray(DefaultChargeKind, ['GiftCard', 'Antivirus', 'InternetPackage']) > -1) {
            $('#chargeform').hide();
            $('#payment').hide();
        }
	});
	...
	$('input#CheckBill').click(function() {
		...
		if (emptyCheck && billIdCheck && paymentIdCheck && cellphoneCheck && emailCheck) {
			...
			$('div.container.Bill div#payment').fadeIn();
	...
	$('div.operator.InternetPackage').click(function() {
		...
		$('div.InternetPackage div.info div#operator').removeClass().addClass('operator InternetPackage ' + $(this).data('type'));
		$('div.InternetPackage div.info div.title').text("بسته اینترنت ایرانسل");
		$('div.InternetPackage div.info div.description').text(InternetPackageKindDescription[InternetPackageKinds.indexOf($(this).data('type'))]);
		...
		$('div.container.InternetPackage div#payment').fadeIn();
    ...
    $('input[type=radio][name=sim-type]').change(function() {
        if (this.value == 'Prepaid') {
            setInternetPackage("mtn", $('div.InternetPackage div.info div#operator').attr('class').replace("operator InternetPackage ", ""), "Prepaid");
        } else if (this.value == 'Postpaid') {
			setInternetPackage("mtn", $('div.InternetPackage div.info div#operator').attr('class').replace("operator InternetPackage ", ""), "Postpaid");
        }
    });
